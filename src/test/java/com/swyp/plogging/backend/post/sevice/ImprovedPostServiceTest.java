...
import com.swyp.plogging.backend.post.post.sevice.PostLockFacade;
...
public class ImprovedPostServiceTest {
    ...
    @Test
    @DisplayName("낙관적락 테스트 - 100회")
    public void test_optimisticLockWith100Times(TestInfo testInfo) throws InterruptedException {
        log.info("=== ({}) 테스트 시작 ===", testInfo.getDisplayName());
        // given
        List<AppUser> users = new ArrayList<>();
        for(int i = 0; i < 100; i++){
        ...
        ConcurrencyUtil.executeConflictingParticipate(
                ...
                (postId, user) -> {
                    try {
                        postLockFacade.participateToPost(postId, user);
